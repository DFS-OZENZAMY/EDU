...
import { BookOpen, UserCheck, Clock, AlertTriangle, Search, Plus, Filter } from "lucide-react"
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-end gap-6 border-b border-gray-200 pb-8">
        <div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">Library Center</h2>
          <p className="text-slate-500 font-bold mt-1 uppercase text-xs tracking-widest">Book Catalog & Borrowing System</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          { label: "Books Borrowed", value: "142", icon: UserCheck, color: "blue" },
          { label: "Overdue Returns", value: "12", icon: AlertTriangle, color: "red" },
          { label: "Total Catalog", value: "2,450", icon: BookOpen, color: "emerald" },
        ].map((k, i) => (
      ...
      <Card className="p-8 rounded-[40px] border-0 shadow-sm">
         <div className="flex items-center justify-between mb-8">
            <h3 className="font-black text-slate-900 text-xl tracking-tight">Active Loans</h3>
            <div className="flex gap-2">
               <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                  <input type="text" placeholder="Search loans..." className="pl-10 pr-4 py-2 bg-gray-50 border-0 rounded-xl text-xs font-bold outline-none" />
               </div>
            </div>
         </div>
